...
#define DATA2_BASE_PIN 6
#define DATA_N_PINS 6

#define ROWSEL_BASE_PIN 18
...
int main() {
    ...
    #ifdef TWO
    uint data_prog_two_offs = pio_add_program(pio_two, &hub75_data_rgb888_program);
    #endif

    hub75_data_rgb888_program_init(pio, sm_data, data_prog_offs, DATA_BASE_PIN, CLK_PIN);
    #ifdef TWO
    hub75_data_rgb888_program_init(pio_two, sm_data, data_prog_two_offs, DATA2_BASE_PIN, CLK2_PIN);
    #endif
    ...
    while (1) {
        for (int rowsel = 0; rowsel < (1 << ROWSEL_N_PINS); ++rowsel) {
            ...
            for (int bit = 0; bit < 8; ++bit) {
                ...
                #ifdef TWO
                    hub75_data_rgb888_set_shift(pio_two, sm_data, data_prog_two_offs, bit);
                #endif
                ...
                #ifdef TWO
                    pio_sm_put_blocking(pio_two, sm_data, gc_row[0][x]);
                    pio_sm_put_blocking(pio_two, sm_data, gc_row[1][x]);
                #endif
                }
                ...
                pio_sm_put_blocking(pio, sm_data, 0);
                #ifdef TWO
                pio_sm_put_blocking(pio_two, sm_data, 0);
                pio_sm_put_blocking(pio_two, sm_data, 0);
                #endif
                ...
                #ifdef TWO
                hub75_wait_tx_stall(pio_two, sm_data);
                #endif
